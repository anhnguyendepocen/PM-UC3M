...
\end{align}
involves inverting the $(p+1)\times(p+1)$ matrix $\mathbf{X}'\mathbf{X}$, where $\mathbf{X}$ is an $n\times(p+1)$ matrix. The vector to be obtained, $\hat{\boldsymbol{\beta}}$, is of size $p+1$. However, computing it directly from \@ref(eq:bd) needs from allocating $\mathcal{O}(np + p^2)$ elements in memory. When $n$ is very large, this can be prohibitive. In addition, for convenience of the statistical analysis, `R`'s `lm` returns several objects of the same size as $\mathbf{X}$ and $\mathbf{Y}$, thus increasing notably the memory usage. For these reasons, alternative approaches for computing $\hat{\boldsymbol{\beta}}$ with big data are required.
...
\end{align}
This has the advantage of not requiring to compute $\mathbf{X}'\mathbf{X}$ and then to invert it. Instead of that, we work directly with $\left(\mathbf{X}_{-i}'\mathbf{X}_{-i}\right)^{-1}$, which ws already computed and is of size $(p + 1)\times(p+1)$.
...
2. Add one of the remaining data points to get $\hat{\boldsymbol{\beta}}_\mathrm{new}$ from \@ref(eq:rec) and \@ref(eq:recmat).
...
p <- 10
x1 <- matrix(rnorm(n * p), nrow = n, ncol = p)
y1 <- 1 + x1 %*% c(1, -1, rep(0, p - 2)) + rnorm(n)
x2 <- matrix(rnorm(100 * p), nrow = 100, ncol = p)
y2 <- 1 + x2 %*% c(1, -1, rep(0, p - 2)) + rnorm(100)
bigData1 <- data.frame("resp" = y1, "pred" = x1)
bigData2 <- data.frame("resp" = y2, "pred" = x2)

# biglm has a very similar syntaxis to lm - but the formula interface is 
# somehow sloppy
library(biglm)
...
# Hack for automated inclusion of all the predictors
...
# However, the reduction in size of the resulting object is more than notable
...
# Must contain a column for the response - even if empty
# predict(lmMod, newdata = bigData2[1:5, -1]) # Error
predict(lmMod, newdata = cbind(0, bigData2[1:5, -1]))
...
# Features not inmediately available for biglm objects: stepwise selection by 
...
```
In addition to this, if `lm` was called, it will return the `residuals`, `effects`, and `fitted.values` slots (all vectors of length $n$, hence $0.7 \times 3=2.1$ Gb more). It will also return the `qr` decomposition of the design matrix and the model matrix (both are $n\times (p + 1)$ matrices, so another $8.2\times 2=14.4$ Gb more). The final `lm` object will be thus at the very least of size $16.5$ Gb. Clearly, this is not a very memory-friendly way of proceeding.

The appropriate approach is to split the dataset and perform updates of the model in chunks of reasonable size. The next code provides a template for such approach using `biglm` and `update`.
