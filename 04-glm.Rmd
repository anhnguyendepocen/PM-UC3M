...
\]
This, in particular, implies that $Y$ is *continuous*. In this chapter we will see how *generalized linear models* can deal with other kind of distributions for $Y|(X_1=x_1,\ldots,X_p=x_p)$, particularly with *discrete* responses, by modelling the *transformed* conditional expectation by a linear combination:
\[
g(\mathbb{E}[Y|X_1=x_1,\ldots,X_p=x_p])=\beta_0+\beta_1x_1+\ldots+\beta_px_p.
\]
The simplest generalized linear model is *logistic regression*, which is arises when $Y$ is a *binary* response, that is, a variable encoding two categories with $0$ and $1$. This model would be useful, for example, to predict $Y$ given $X$ from a sample $(X_1,Y_1),\ldots,(X_n,Y_n)$ such that its scatterplot is the one in Figure \@ref(fig:logrel).
...
## Model formulation and estimation by maximum likelihood {#glm-model}
...
Generalized linear models are intimately related with the **exponential family**, which is the family of distributions with pdf expressable as
\begin{align}
f(y;\theta,\phi)=\exp\left\{\frac{y\theta-b(\theta)}{a(\phi)}+c(y,\phi)\right\},(\#eq:expd)
\end{align}
where $a(\cdot)$, $b(\cdot)$, and $c(\cdot,\cdot)$ are specific functions. If $Y$ has the pdf \@ref(eq:expd), then we write $Y\sim\mathrm{E}(\theta,\phi)$. If  $\phi$ is known, this is an exponential family with *canonical localization parameter* $\theta$ (if $\phi$ is unknown, then it may or not may be a two-parameter exponential family). Distributions from the exponential family have some nice properties. If $Y\sim\mathrm{E}(\theta,\phi)$, then
\[
...
\]
In the case of canonical link function, the one-line summary of the generalized model is (independence is implicit)
\begin{align}
Y|(X_1=x_1,\ldots,X_p=x_p)\sim\mathrm{E}(g(\beta_0+\beta_1x_1+\ldots+\beta_px_p),\phi).(\#eq:expglm)
\end{align}

```{block2, exexpfam, type = 'rmdexercise', cache = TRUE}
Show that the Normal, Bernouilli, and Poisson distributions are members of the exponential family. For that, express their pdfs in terms of \@ref(eq:expd) and identify who is $\theta$ and $\phi$.
```
...
\[
Y_i|(X_1=x_{i1},\ldots,X_p=x_{ip})\sim\mathrm{E}(\theta_i,\phi),\quad i=1,\ldots,n,
\]
...
legend("topright", legend = c("Low pH", "Medium pH", "High pH"), col = 1:3)
...
lines(bio, exp(species2$coefficients[4] + species2$coefficients[6] * bio  + z),
      col = 3, lty = 2)
# Seems to provide a significant improvement
```
...
- **Variance**. It depends on:
    ...
    - *Weighted predictor sparsity $(\mathbf{X}^T\mathbf{V}\mathbf{X})^{-1}$*. The more *sparse* the predictor is (small eigenvalues of $(\mathbf{X}^T\mathbf{V}\mathbf{X})^{-1}$), the more precise $\hat{\boldsymbol{\beta}}$ is.
...
H_0:\beta_j=0
\end{align*}
for $j=0,\ldots,p$. The test of $H_0:\beta_j=0$ with $1\leq j\leq p$ is especially interesting, since it allows to answer whether *the variable $X_j$ has a significant effect on $\mathbb{P}[Y=1]$*. The statistic used for testing for significance is the Wald statistic
\begin{align*}
...
## Deviance, diagnostics, and model selection {#glm-deviance}
...
```{r, glmshr-1, echo = TRUE, collapse = TRUE, cache = TRUE, fig.asp = 1/2, out.width = '70%', fig.pos = 'h!'}
...
These two peculiarities are a game-changer for the approach followed in \@ref(lm-iii-bigdata): `biglm`'s fitting function for generalized linear models, `bigglm`, needs to have access to the full data while performing the fitting. This can be cumbersome. 
...
# the true BICs of the generalized models, but a sufficient approximation to
# obtain a list of candidate models in a fast way
...
# Let's compute the true BICs for the 9 models. This implies fitting 9 bigglm's
bestModels <- list()
