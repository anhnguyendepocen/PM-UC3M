...
\]
that is, how the conditional expectation of $Y$ or, equivalently, the conditional probability of $Y=1$, is changing according to particular values of the predictors. At sight of \@ref(eq:eq-lm), a tempting possibility is to consider the model
\[
...
\end{align}
The conditional log-odds \@ref(eq:eq-odds-log2) here plays the role of the conditional mean for multiple linear regression. Therefore, we have an analogous interpretation for the coefficients:
...
The log-odds is not as easy to interpret as the odds. For that reason, an equivalent way of interpreting the coefficients, this time based on \@ref(eq:eq-odds-log1), is:
...
| $0,1$ | $\mathrm{B}(1,p)$ | $\mathrm{logit}(\mu)$ | $\mathrm{logistic}(\eta)$ | $1$ | $\mathrm{B}\left(1,\mathrm{logistic}(\eta)\right)$ |
...
The distributions in \@ref(eq:mle3) also allow us to conduct a formal hypothesis test on the coefficients $\beta_j$, $j=0,\ldots,p$. For example, the test for significance:
\begin{align*}
H_0:\beta_j=0
\end{align*}
...
\[
\hat{\mathbb{P}}[Y=1|X_1=X_{i1},\ldots,X_k=X_{ip}]=Y_i,\quad i=1,\ldots,n.
\]
...
\end{align}
In most of the cases, $a(\phi)\propto\phi$, so the deviance *does not depend on $\phi$*. Expression \@ref(eq:dev) is interesting, since it allows the following key insight:
...
\end{align*}
since $\hat\eta_i=\hat\beta_0+\hat\beta_1x_{i1}+\ldots+\hat\beta_px_{ip}$. Remember that $\mathrm{RSS}(\hat{\boldsymbol{\beta}})$ is just another name for the SSE.
...
\text{Perfect fit} \\
\text{(interpolation)} 
\end{array}\\}_{\LARGE D}\\}_{\LARGE D_0}
]{\Large\text{Likelihood sorting of the model space}}
\]
...
\end{align}
where $\chi^2_{k}$ is the *Chi-squared distribution with $k$ degrees of freedom*. In the case of the linear model, $D^*=\frac{1}{\sigma^2}\mathrm{RSS}$ is *exactly* distributed as a $\chi^2_{n-p-1}$. The result \@ref(eq:dp) provides a way of estimating $\phi$ when it is unknown: match $D^*=\frac{D}{\phi}$ with the expectation $\mathbb{E}\left[\chi^2_{n-p-1}\right]=n-p-1$. This provides
\[
...
\begin{align}
D^*_{p_1}-D^*_{p_2}\stackrel{H_0}{\sim}\chi^2_{p_2-p_1}.(\#eq:d12)
\end{align}
If $H_0$ is true, then $D_{p_1}^*-D^*_{p_2}$ is expected to be *small*, thus we will reject $H_0$ if the value of the statistic is above the *$\alpha$-upper quantile of the $\chi^2_{p_2-p_1}$*, denoted as $\chi^2_{\alpha;p_2-p_1}$.

Note that $D^*$ apparently removes the effects of $\phi$, but it is still dependent on $\phi$, since this is hidden in the likelihood (see \@ref(eq:dev)). Therefore, $D^*$ cannot be computed unless $\phi$ is know, which forbids using \@ref(eq:d12). Hopefully, this dependence is removed by employing \@ref(eq:dp) and \@ref(eq:d12) and assuming that they are asymptotically independent. This gives the $F$-test for $H_0$:
\begin{align*}
F=\frac{(D^*_{p_1}-D^*_{p_2})/(p_2-p_1)}{D^*_{p_2}/(n-p_2-1)}=\frac{(D_{p_1}-D_{p_2})/(p_2-p_1)}{D_{p_2}/(n-p_2-1)}=\stackrel{H_0}{\sim} F_{p_2-p_1,n-p_2-1}.
\end{align*}
Note that the LHS is perfectly computable, since $\phi$ cancels due to the quotient (and because we assume that $a(\phi)\propto\phi$). Note also that this is an extension of the $F$-test as we saw it in Section \@ref(lm-i-anova): take $p_1=0$ and $p_2=p$ and you test for the significance of all the predictors included in the model (both models contain intercept).
...
The same discussion we did in Section \@ref(lm-ii-modsel) is applicable to generalized linear models with small changes:
...
Stepwise selection can be done through `stepAIC` as in linear models^[The `leaps` package does not support generalized linear models directly. There are, however, other packages for performing best subset selection with generalized linear models, but we do not cover them here.]. Conveniently `summary` also reports the AIC:
...
The **deviance residuals** are the generalization of the residuals $\hat\varepsilon_i=Y_i-\hat Y_i$ from the linear model. They are constructed using the analogy between the deviance and the RSS saw in \@ref(eq:devrss). The deviance can be expressed into a sum of terms associated to each datum (recall e.g. \@ref(eq:dev)):
\[
D=\sum_{i=1}^n d_i.
\]
...
\]
and have a generalization of $\hat\varepsilon_i$ for generalized linear models. This definition has interesting distributional consequences. From \@ref(eq:dp), we know that, asymptotically, $D^*\sim\chi_{n-p-1}^2$. This *suggests* that^[Remember: a $\chi^2_k$ random variable has the same distribution as the sum of $k$ independent squared $\mathcal{N}(0,1)$ variables.] (practical counterexamples are possible, see Figures \@ref(fig:poisdiagnostics2-good) and \@ref(fig:logdiagnostics2-good))
\[
\hat\varepsilon_i^D\text{ are approximately normal}.
\]
The previous statement is of variable accuracy, depending on the model, sample size, and distribution of the predictors. In the linear model, it is exact and $(\hat\varepsilon_1^D,\ldots,\hat\varepsilon_n^D)$ are distributed *exactly* as a $\mathcal{N}_n(\mathbf{0},\sigma^2\mathbf{X}'(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X})$.
...
- If the canonical function is employed, then $\sum_{i=1}^n \hat\varepsilon_i^D=0$, as in linear models.
...
The script used for generating the following Figures \@ref(fig:poisdiagnostics1-good)--\@ref(fig:logdiagnostics3-bad) is available [here](https://github.com/egarpor/PM-UC3M/blob/master/images/R/hypothesisGlm.R).
...
We use the *residuals vs. fitted values plot*, which for generalized linear models is the scatterplot of $(\hat\eta_i,\hat \varepsilon_i^D)$, $i=1,\ldots,n$. Recall that it is **not** the scatterplot of $(\hat Y_i,\hat \varepsilon_i)$. Under linearity, we expect that there is *no trend in the residuals* $\hat\varepsilon_i^D$ with respect to $\hat \eta_i$. If nonlinearities are observed, it is worth plotting the regression terms of the model via `termplot`.

(ref:poisdiagnostics1-good-title) Residuals vs. fitted values plots (first row) for datasets (second row) **respecting** the linearity assumption in **Poisson** regression.

(ref:poisdiagnostics1-bad-title) Residuals vs. fitted values plots (first row) for datasets (second row) **violating** the linearity assumption in **Poisson** regression.

(ref:logdiagnostics1-good-title) Residuals vs. fitted values plots (first row) for datasets (second row) **respecting** the linearity assumption in **logistic** regression.

(ref:logdiagnostics1-bad-title) Residuals vs. fitted values plots (first row) for datasets (second row) **violating** the linearity assumption in **logistic** regression.

```{r, poisdiagnostics1-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics1-good-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag1-good.png")
```

```{r, poisdiagnostics1-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics1-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag1-bad.png")
```

```{r, logdiagnostics1-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics1-good-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag1-good.png")
```

```{r, logdiagnostics1-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics1-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag1-bad.png")
```
...
(ref:poisdiagnostics2-good-title) QQ-plots for the deviance residuals (first row) for datasets (second row) **respecting** the response distribution assumption for **Poisson** regression.

(ref:poisdiagnostics2-bad-title) QQ-plots for the deviance residuals (first row) for datasets (second row) **violating** the response distribution assumption for **Poisson** regression.

(ref:logdiagnostics2-good-title) QQ-plots for the deviance residuals (first row) for datasets (second row) **respecting** the response distribution assumption for **logistic** regression.

(ref:logdiagnostics2-bad-title) QQ-plots for the deviance residuals (first row) for datasets (second row) **violating** the response distribution assumption for **logistic** regression.

```{r, poisdiagnostics2-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics2-good-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag2-good.png")
```

```{r, poisdiagnostics2-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics2-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag2-bad.png")
```

```{r, logdiagnostics2-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics2-good-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag2-good.png")
```

```{r, logdiagnostics2-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics2-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag2-bad.png")
```
...
(ref:poisdiagnostics3-good-title) Serial plots of the residuals (first row) for datasets (second row) **respecting** the independence assumption for **Poisson** regression.

(ref:poisdiagnostics3-bad-title) Serial plots of the residuals (first row) for datasets (second row) **violating** the independence assumption for **Poisson** regression.

(ref:logdiagnostics3-good-title) Serial plots of the residuals (first row) for datasets (second row) **respecting** the independence assumption for **logistic** regression.

(ref:logdiagnostics3-bad-title) Serial plots of the residuals (first row) for datasets (second row) **violating** the independence assumption for **logistic** regression.

```{r, poisdiagnostics3-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics3-good-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag3-good.png")
```

```{r, poisdiagnostics3-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:poisdiagnostics3-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/poisdiag3-bad.png")
```

```{r, logdiagnostics3-good, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics3-good-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag3-good.png")
```

```{r, logdiagnostics3-bad, echo = FALSE, out.width = '90%', fig.pos = 'h!', fig.cap = '(ref:logdiagnostics3-bad-title)', cache = TRUE}
knitr::include_graphics("images/R/logdiag3-bad.png")
```

**Under uncorrelation, we expect the series to show no *tracking* of the residuals**, which is a sign of positive serial correlation. Negative serial correlation can be identified in the form of a small-large or positive-negative systematic alternation of the residuals. This can be explored better with `lag.plot`, as saw in Section \@ref(lm-ii-diagnostics-4).
