...
Along this section we will make the following assumptions^[These are the only assumptions done do far in the model (in addition to $\varepsilon$ independent from $X$).]:
...
\end{align}
Each evaluation of $\hat m_{\ell}(x;h,p)$ in a different $x$ requires, thus, from a fit of the underlying logistic model.
