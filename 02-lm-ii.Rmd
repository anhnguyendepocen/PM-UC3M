...
\vdots & \vdots   & \ddots & \vdots \\
p_1(X_{n}) & p_1(X_{n}) & \ldots & p_k(X_{n}) \\
\end{pmatrix},
$$
...
mod2Raw <- lm(dist ~ poly(speed, degree = 2, raw = 2), data = cars)
plot(cars, xlab = "Speed (mph)", ylab = "Stopping distance (ft)")
...
plot(medv ~ lstat, data = Boston, col = chas + 3, pch = 16, cex = 0.25)
abline(coef = mod1$coefficients, col = 2)
...
summary(mod3 <- lm(medv ~ lstat + lstat:chas, data = Boston)) # 2
plot(medv ~ lstat, data = Boston, col = chas + 3, pch = 16, cex = 0.25)
...
summary(mod4 <- lm(medv ~ lstat * chas, data = Boston)) # 3
plot(medv ~ lstat, data = Boston, col = chas + 3, pch = 16, cex = 0.25)
...
**Under linearity, we expect the red line** (a flexible fit of the mean of the residuals) **to capture no trend**. If nonlinearities are observed, it is worth to inspect the most suspicious pairs $(Y,X_j)$ by making the scatterplot of $(\hat \varepsilon_i,X_{ij})$, $i=1,\ldots,n$. For example:

```{r, collapse = TRUE, cache = TRUE, fig.asp = 1}
plot(mod$model[, 1], mod$residuals) # First predictor
```

Another possibility is to plot the *regression terms* of the model. These are the $p$ scatterplots $(Y_i,X_{ij})$, $i=1,\ldots,n$, that are accompanied by the regression lines $y=\hat\beta_0+\hat\beta_jx$ (important: $\hat\beta_j$, $j=1,\ldots,p$, come from the multiple linear fit that gives $\hat{\boldsymbol\beta}$). They help on detecting which predictor is having  nonlinearity effects on $Y$.
```{r, collapse = TRUE, cache = TRUE, fig.asp = 1}
...
Patching non-normality is not easy and requires from the consideration of other models, like the ones to be seen in Chapter \@ref(glm). It is possible to patch it if it is a consequence of the failure of linearity or homoscedasticity, which translates the problem into fixing those assumptions.
