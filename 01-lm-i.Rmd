...
```{r, lscheck-2, collapse = TRUE, cache = TRUE}
# Call lm
...
```{block2, exdistances, type = 'rmdexercise'}
...
```{block, caupopreg, type = 'rmdcaution'}
...
```{r, wintab-2, collapse = TRUE, cache = TRUE, fig.asp = 1}
...
```{block2, insdesign1, type = 'rmdinsight'}
...
```{block2, insestcoefinterp, type = 'rmdinsight'}
...
```{block2, excheck, type = 'rmdexercise'}
...
```{block, insjointmargcoef, type = 'rmdinsight'}
...
```{block2, insrempreds, type = 'rmdinsight'}
Recall:
...
```{block, exassump-1, type = 'rmdexercise'}
...
```{block, exassump-2, type = 'rmdexercise'}
...
```{block, caunamespred, type = 'rmdcaution'}
...
```{block2, exwine, type = 'rmdexercise'}
...
```{block, cauanova, type = 'rmdcaution'}
...
```{block, exanova, type = 'rmdexercise'}
...
```{block, exassump-3, type = 'rmdexercise'}
...
```{block2, insR2connection, type = 'rmdinsight'}
...
```{block, insR2RSS, type = 'rmdinsight'}
...
```{block, cauR2Adj, type = 'rmdcaution'}
...
```{block, expreds, type = 'rmdexercise'}
