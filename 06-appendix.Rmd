...
# Least squares and maximum likelihood estimation {#app-mle}
...
```{block, insmlelog, type = 'rmdinsight', cache = TRUE}
Maximum likelihood estimation is **asymptotically optimal** when estimating unknown parameters of a model. This is a very appealing property which means that, when the sample size $n$ is large, it is **guaranteed to perform *better* than any other estimation method**, where *better* is understood in terms of the measn squared error.
```
...
# Multinomial logistic regression {#app-multinomialreg}

The logistic model seen in Section \@ref(glm-model-log) can be generalized to categorical variables $Y$ with more than two possible levels, namely $\{1,\ldots,J\}$. Given the predictors $X_1,\ldots,X_p$, *multinomial logistic regression* models the probability of each level $j$ of $Y$ by
\begin{align}
p_j(\mathbf{x}):=\mathbb{P}[Y=j|X_1=x_1,\ldots,X_p=x_p]=\frac{e^{\beta_{0j}+\beta_{1j}X_1+\ldots+\beta_{pj}X_p}}{1+\sum_{l=1}^{J-1}e^{\beta_{0l}+\beta_{1l}X_1+\ldots+\beta_{pl}X_p}} (\#eq:multinom-1)
\end{align}
for $j=1,\ldots,J-1$ and (for the last level $J$)
\begin{align}
p_J(\mathbf{x}):=\mathbb{P}[Y=J|X_1=x_1,\ldots,X_p=x_p]=\frac{1}{1+\sum_{l=1}^{J-1}e^{\beta_{0l}+\beta_{1l}X_1+\ldots+\beta_{pl}X_p}}. (\#eq:multinom-2)
\end{align}
Note that \@ref(eq:multinom-1) and \@ref(eq:multinom-2) imply that $\sum_{j=1}^J p_j(\mathbf{x})=1$ and that there are $(J-1)\times(p+1)$ coefficients ($(J-1)$ intercepts and $(J-1)\times p$ slopes). Also, \@ref(eq:multinom-2) reveals that the last level, $J$, is given a different treatment. This is because it is the *reference level* (it could be a different one, but is tradition to choose the last one).
...
\begin{align}
\frac{p_j(\mathbf{x})}{p_J(\mathbf{x})}=e^{\beta_{0j}+\beta_{1j}X_1+\ldots+\beta_{pj}X_p}(\#eq:multinom-4)
\end{align}
for $j=1,\ldots,J-1$. Therefore, applying a logarithm to both sides we have:
\begin{align}
\log\frac{p_j(\mathbf{x})}{p_J(\mathbf{x})}=\beta_{0j}+\beta_{1j}X_1+\ldots+\beta_{pj}X_p.(\#eq:multinom-3)
\end{align}
...
\[
p_j(\mathbf{x})=e^{\beta_{0j}+\beta_{1j}X_1+\ldots+\beta_{pj}X_p}p_J(\mathbf{x}).
\]
Therefore:
...
```{block, caumlrnum, type = 'rmdcaution', cache = TRUE}
...
2.  Generalized linear models **or**^[This is a `|`, not an `xor`.] Nonparametric regression (weight: 40%; **deadline: 2018/01/25**).
